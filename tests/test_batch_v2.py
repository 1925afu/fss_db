...
async def test_batch():
    ...
    try:
        ...
        test_files = [os.path.join(test_dir, f) for f in sorted(all_files)[:5]]  # 처음 5개만
        
        logger.info(f"=== V2 파이프라인 배치 테스트 ===")
        ...
        if results['success']:
            logger.info("\n=== 성공 샘플 ===")
            sample = results['success'][0]
            decision_data = sample['decision_data']
            logger.info(f"의결서: {decision_data['decision_year']}-{decision_data['decision_id']}")
            logger.info(f"제목: {decision_data['title']}")
            logger.info(f"조치 수: {len(decision_data['actions'])}")
            
            if decision_data['actions']:
                action = decision_data['actions'][0]
                logger.info(f"첫 번째 조치:")
                logger.info(f"  - 대상: {action['entity_name']}")
                logger.info(f"  - 유형: {action['action_type']}")
                logger.info(f"  - 요약: {action['violation_summary'][:100]}...")
