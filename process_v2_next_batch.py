...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'process_v2_next_batch_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
async def main():
    ...
    for filename in RETRY_FILES:
        ...
        if pdf_path.exists():
            decision_num = int(filename.split("제2025-")[1].split("호")[0])
            if decision_num not in processed_ids:
                pdf_files.append(pdf_path)
                logger.info(f"재시도 파일 추가: {filename}")
    ...
    # 20개만 선택
    batch_size = 20
    pdf_files = pdf_files[:batch_size]
    logger.info(f"{len(pdf_files)}개 파일 처리 예정")
    ...
    try:
        ...
        # 남은 파일 수 계산
        remaining = len(all_pdf_files) - total_v2_decisions - skipped_count
        if remaining > 0:
            logger.info(f"남은 파일: {remaining}개 (문제 파일 2개 제외)")
    finally:
        db.close()
