...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'process_v2_skip_problematic_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
async def main():
    ...
    try:
        ...
        if remaining > 0:
            logger.info(f"남은 파일: {remaining}개 (문제 파일 {skipped_count}개 제외)")
    finally:
        db.close()
